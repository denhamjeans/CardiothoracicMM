...
slide1.plotdata %<>% 
  mutate(Percentage = Count/sum(slide1.plotdata$Count)*100, 1) %>% 
  mutate(Percentage_Count = paste(Count, " - ", round(Count/sum(slide1.plotdata$Count)*100, 1), "%"))  
...
slide3.data <- slide3.data %>% my_kable()
```
...
```{r ICU Timeintervals}
MM.Pres_Main[MM.Pres_Main$Pt_LName == 'NEAL, H',"DischargeDate"] = as.Date('2019-02-07')
MM.Pres_Main[MM.Pres_Main$Pt_LName == 'LOVETT, S',"AdmDate"] = as.Date('2019-01-01')


MM.Pres_Main$Pt_LName <- factor(MM.Pres_Main$Pt_LName, levels = (MM.Pres_Main$Pt_LName[order(MM.Pres_Main$OpDate)]))
MM.Pres_Main$Pt_LName

ggplot(MM.Pres_Main, aes(y = Pt_LName)) +
  geom_segment(aes(x = AdmDate, xend = DischargeDate, y = Pt_LName, yend = Pt_LName)) +
  geom_segment(aes(x = PO_ICUAdmDt, xend = PO_ICUOutDt, y = Pt_LName, yend = Pt_LName), size = 5, colour = "#D16103") +
  geom_segment(aes(x = OpDate, xend = OpDate + 0.5, y = Pt_LName, yend = Pt_LName), size = 5, colour = "#4E84C4") +
  geom_segment(aes(x = AdmDate, xend = AdmDate+0.1, y = Pt_LName, yend = Pt_LName), size = 5) +
  ...
  scale_y_discrete(breaks=MM.Pres_Main$Pt_Lname, labels = MM.Pres_Main$Pt_Lname) +
