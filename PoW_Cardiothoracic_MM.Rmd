...
Cockcroft_Gault <- modules::use('EuroScore.R')[['Cockcroft_Gault']]
EuroScoreAdd.I <- modules::use('EuroScore.R')[['EuroScoreAdd.I']]
EuroScoreLog.I <- modules::use('EuroScore.R')[['EuroScoreLog.I']]
EuroScoreLog.II <- modules::use('EuroScore.R')[['EuroScoreLog.II']]
```
...
if(DatabaseConnection == TRUE){
  ...
  #### FOR REMOTE DATABASE
  MM.Inf <-       dbGetQuery(con, glue_sql(getSQL('Query\\qry_MMPres_Inf.sql'), 
                                          MinDate = Min_Date,
                                          MaxDate = Max_Date,
                                          .con = con
                                          ))
  MM.Pres_Main <- dbGetQuery(con, glue_sql(getSQL('Query\\qry_MMPres_Main.sql'), 
                                          MinDate = Min_Date,
                                          MaxDate = Max_Date,
                                          .con = con
                                          ))
  MM.ReAdm <-     dbGetQuery(con, glue_sql(getSQL('Query\\qry_MMPres_ReAdm.sql'), 
                                          MinDate = Min_Date,
                                          MaxDate = Max_Date,
                                          .con = con
                                          ))
  MM.RTT <-       dbGetQuery(con, glue_sql(getSQL('Query\\qry_MMPres_RTT.sql'), 
                                          MinDate = Min_Date,
                                          MaxDate = Max_Date,
                                          .con = con
                                          ))
...
MM.Pres_Main[, unlist(lapply(dplyr::select_if(MM.Pres_Main), is.integer64))]
  
MM.Pres_Main %<>% type_convert(col_types = cols(
    TF_Hospital = readr::col_factor()
  ))
  
  MM.Pres_Main$Adm_Hospital <- as.factor(MM.Pres_Main$Adm_Hospital)
  MM.Pres_Main$AdmDate <- as.Date(MM.Pres_Main$AdmDate, format = "%d/%m/%Y")
  MM.Pres_Main$ConsultDate <- as.Date(MM.Pres_Main$ConsultDate, format = "%d/%m/%Y")
  MM.Pres_Main$OpDate <- as.Date(MM.Pres_Main$OpDate, format = "%d/%m/%Y")
  MM.Pres_Main$DischargeDate <- as.Date(MM.Pres_Main$DischargeDate, format = "%d/%m/%Y")


  
#create Renal EuroScore variable 
#Only works for Database Connection
MM.Pres_Main %<>% 
  dplyr::mutate(EuroScore_ri = dplyr::if_else(EuroScore_ri_dialysis==1, 4,
                               dplyr::if_else(EuroScore_ri_creatinine_clearance<50 & EuroScore_ri_dialysis==0, 3,
                               dplyr::if_else(EuroScore_ri_creatinine_clearance>=50 & EuroScore_ri_creatinine_clearance<=85 & EuroScore_ri_dialysis==0, 2, 
                               dplyr::if_else(EuroScore_ri_creatinine_clearance>85 & EuroScore_ri_dialysis==0, 1, 0)))))
  
Euroscore_vars <- MM.Pres_Main[,c("EuroScore_age", "EuroScore_sex", "EuroScore_ri", "EuroScore_eca", "EuroScore_pm", "EuroScore_pcs", "EuroScore_cld", "EuroScore_ae", "EuroScore_cps", "EuroScore_doi", "EuroScore_nyha", "EuroScore_ccs", "EuroScore_lv_ef", "EuroScore_rmi", "EuroScore_ph", "EuroScore_em", "EuroScore_woi", "EuroScore_sta")]

colnames(Euroscore_vars) <- c("age", "sex", "ri", "eca", "pm", "pcs", "cld", "ae", "cps", "doi", "nyha", "ccs", "lv.ef", "rmi", "ph", "em", "woi", "sta")

##########################################################REMOVE NA=0 WHEN DATABASE IS FUNCTIONING CORRECTLY
Euroscore_vars[is.na(Euroscore_vars)] = 0

EuroScore_II = EuroScoreLog.II(Euroscore_vars)
  
  
}else{

  #### FOR LOCAL CSV Files
  #LocalPathFile <- "D:M&M_Query_Data_Laura/Oct_Nov/"
  ...
  MM.Pres_Main$PO_ICUAdmTm <- as.POSIXct(MM.Pres_Main$PO_ICUAdmTm, format = "%d/%m/%Y %H:%M:%S") %>%
  ...
  #IABP In DATE/TIME
  MM.Pres_Main$IABP_InDt <- as.Date(MM.Pres_Main$IABP_InDt, format = "%d/%m/%Y")
  ...
  #IABP Out DATE/TIME
  MM.Pres_Main$IABP_OutDt <- as.Date(MM.Pres_Main$IABP_OutDt, format = "%d/%m/%Y")
  ...
  #IDC Out DATE/TIME
  MM.Pres_Main$PO_IDCOutDate <- as.Date(MM.Pres_Main$PO_IDCOutDate, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$IntubationDt <- as.Date(MM.Pres_Main$IntubationDt, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$PO_ExtbDt <- as.Date(MM.Pres_Main$PO_ExtbDt, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$PO_ReIntbDt <- as.Date(MM.Pres_Main$PO_ReIntbDt, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$PO_ReExtbDt <- as.Date(MM.Pres_Main$PO_ReExtbDt, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$PO_RFWDt <- as.Date(MM.Pres_Main$PO_RFWDt, format = "%d/%m/%Y")
  ...
  MM.Pres_Main$PO_ICUOutDt <- as.Date(MM.Pres_Main$PO_ICUOutDt, format = "%d/%m/%Y")
...
###DROP TIME VARIABLES (Combined above)####


drops <- c("PO_ICUAdmTm", "PO_ICUOutTm", "PO_RFW", "IABP_OutTm", "IABP_InTm", "IntubationTm", "PO_ExtbTm", "PO_IDC_OutTime", "PO_RFWTm", "PO_ReIntbTm", "PO_ReExtbTm")
MM.Pres_Main <- MM.Pres_Main[ , !(names(MM.Pres_Main) %in% drops)]
...
#Calculate 24 Hour Ventilation as Binary
test <- MM.Pres_Main %>% mutate(`PO_Vent>24hr` = if_else(`POST_OP_VENT_DURATION`>=24, 1, 0))
