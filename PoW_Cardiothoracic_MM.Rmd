...
slide15.data1 <- slide15.data2 %>%  
  group_by(Month) %>% 
  summarise(`Recieved Blood Product` = paste(sum(`Recieved_Bld_Prod`), ' (', round(sum(`Recieved_Bld_Prod`)/n()*100, 2),'%)', sep = ""),
            `RBC > 4 Units` = paste(sum(`RBC > 4 U`), ' (', round(sum(`RBC > 4 U`)/n()*100, 2),'%)', sep = ""),
            `RBC > 15 Units` = paste(sum(`RBC > 15 U`), '(', round(sum(`RBC > 15 U`)/n()*100, 2), '%)', sep = ""),
            `Any Blood Prod > 15 Units` = paste(sum(`Any Blood Prod > 15 U`), ' (', round(sum(`Any Blood Prod > 15 U`)/n()*100, 2), '%)', sep = "")) %>%
