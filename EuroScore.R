...
Cockcroft_Gault <- function(age_yrs, weight_kg, female, creatinine) {
  #(140-age (years))   *   weight (kg)   *   (0.85 if female)    /   [72 x serum creatinine (mg/dl)]
  
  if(creatinine < 62 | creatinine > 155){
    warning(paste(Creatinine, "Outside normal Range (62 - 155) μmol/L"))
  }else{
    
    CrCl <- NA
    
    if(female==TRUE){
      CrCl <- (140-age_yrs)*weight_kg*0.85*(creatinine*72)
    }else{
      CrCl <- (140-age_yrs)*weight_kg*(creatinine*72)
    }
    return(CrCl)
  }
}
